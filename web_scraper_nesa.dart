...
/*
  ...
  Future<Map<String, dynamic>> getMarksData() async {
    ...
    bool isDate(String date) {
      if (date.length == 10) {
        if (date.contains('.')) {
          String newString = date.replaceAll('.', '');
          if (newString.length == 8) {
            print(date + ' <-- ist ein Datum...');
            return true;
          }
        }
      }
  ...
  void getAllMarks() async {
    ...
    bool isNoteBegin = false;
    int counter3 = 0;
    List listOfMainMarks = noten.values.toList();
    List newListOfMainMarks = [];
    for (Map<String, dynamic> item in listOfMainMarks) {
      newListOfMainMarks.add(item['Fach']);
    }
    for (Map<String, dynamic> item in listMarks) {
      String titleOfItem = item['title'];

      if (titleOfItem == 'Gewichtung') {
        print('Neues Fach');
        isNoteBegin = true;
        continue;
      } else if (isDate(titleOfItem)) {
        print('Datum wurde gefunden, aber kein neues Fach');
        isNoteBegin = true;
      }

      if (isNoteBegin) {
        if (!isDate(titleOfItem)) {
          isNoteBegin = false;
          continue;
        }
        switch (counter3) {
          ...
          case 1:
            ...
            break;
          case 2:
            valuation = titleOfItem;
            print(titleOfItem);
            counter3++;
            break;
          case 3:
            weighting = titleOfItem;
